...
import { Search, ShoppingCart, Info, Plus, Minus, X, Weight, ArrowLeft, Star, Settings, Trash2, Edit2, Lock, ChevronDown, Heart, Repeat, Check, Share2, MessageCircle, Package, ShieldCheck, Truck, Monitor } from 'lucide-react';
              ...
              <div key={product.id} className="bg-[#121212] rounded-[40px] overflow-hidden border border-white/5 flex h-64 group hover:border-[#f4d300]/20 transition-all duration-500 shadow-2xl relative">
                ...
                <div className="w-3/5 p-6 flex flex-col justify-between">
                  <div>
                    ...
                    <h3 className="brand-font text-2xl font-bold text-white leading-tight group-hover:text-[#f4d300] transition-colors">{product.name}</h3>
              ...
              <div key={p.id} className="max-h-[90vh] overflow-y-auto no-scrollbar">
                <div className="h-[300px] relative">
                  ...
                  <div className="absolute bottom-10 left-10">
                    ...
                    <h2 className="brand-font text-5xl font-bold italic text-white tracking-tighter">{p.name}</h2>
                  </div>
                </div>
                <div className="p-12 space-y-10">
                  <p className="text-white/60 font-medium text-lg leading-relaxed">{p.description}</p>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-6 bg-white/5 p-6 rounded-3xl border border-white/10 mt-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center gap-2 bg-black/50 p-1.5 rounded-xl border border-white/10 h-[52px]">
                                        <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="w-10 h-10 flex items-center justify-center bg-white/10 rounded-lg"><Minus size={16} /></button>
                                        <span className="flex-1 text-lg font-bold text-center">{quantity}</span>
                                        <button onClick={() => setQuantity(q => q + 1)} className="w-10 h-10 flex items-center justify-center bg-white/10 rounded-lg"><Plus size={16} /></button>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 mt-2">
                                    {[250, 500, 1000, 2000].map(w => (
                                        <button key={w} onClick={() => { setIsCustomWeight(false); setWeightGrams(w); }} className={`px-4 py-2 rounded-lg text-xs font-bold border transition-all ${weightGrams === w && !isCustomWeight ? 'bg-[#f4d300] border-[#f4d300] text-black' : 'bg-white/5 border-white/10 text-white/60'}`}>{w}g</button>
                                    ))}
                                </div>
                            </div>
                            <div className="space-y-2">
